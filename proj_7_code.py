...
df=pd.read_csv(r"./proj7_ex.csv")
plot_df=pd.read_csv(r"./proj7_ex.csv")
...
heat_map = px.imshow(df[:-1].corr().round(3), text_auto=True, aspect="auto",width=700, height=700)
heat_map.update_layout(xaxis = dict(tickfont = dict(size=18)))
heat_map.update_layout(yaxis = dict(tickfont = dict(size=18)))

heat_map.update_traces(textfont_size=10)
st.write(heat_map)
...
conf_mat = px.imshow(bg_gbc_conf, text_auto=True, aspect="auto",width=800, height=800)
conf_mat.update_layout(xaxis = dict(tickfont = dict(size=18)))
conf_mat.update_layout(yaxis = dict(tickfont = dict(size=18)))

conf_mat.update_traces(textfont_size=15)
st.write(conf_mat)
